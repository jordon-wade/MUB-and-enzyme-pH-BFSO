...
setwd("~/Dropbox")
MUB.pH.data <- read.csv("Wade Margenot/MUB and pH/Revision/MUB vs pH data_final.csv")
MUB.pH.data <- MUB.pH.data[-100,] # bad data point - uncertain value as per lab notes
MUB.pH.data$pH.change <- MUB.pH.data$pH.t60 - MUB.pH.data$pH.t0 # Change in pH over time
MUB.pH.data$pH.diff0 <- MUB.pH.data$pH.t0 - MUB.pH.data$Target.pH # Difference from target at t = 0 min
MUB.pH.data$pH.diff60 <- MUB.pH.data$pH.t60 - MUB.pH.data$Target.pH # Difference from target at t = 60 min
...
Figure1
ggsave("Figure 1 - pH change by soil.pdf", width=7, height=7)
